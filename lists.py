...
def join_lists(main_list, subordinate_list, keys_list):
    subordinate_dict = dict((item['id'], item) for item in subordinate_list)
    arr = list()
    for item in main_list:
        augmentable = item
        for k in keys_list:
            if item['id'] in subordinate_dict:
                augmentable[k] = subordinate_dict[item['id']][k]
